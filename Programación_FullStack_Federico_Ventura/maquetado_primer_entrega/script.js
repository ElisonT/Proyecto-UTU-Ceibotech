...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (btnBuscarHero) {
    btnBuscarHero.addEventListener('click', (evento) => {
      ...
      window.location.href = 'busqueda.html' + (query ? '?' + query : '');
    });
  }
  ...
  // Barra de duración estimada: cada posición del slider representa una duración real en días.
  // Los primeros pasos son de a 1 día (para elegir con precisión "1, 2, 3 días..."),
  // y más adelante los saltos son más grandes, hasta llegar a 1 año.
  const pasosDuracion = [
    { dias: 1, texto: '1 día' },
    { dias: 2, texto: '2 días' },
    { dias: 3, texto: '3 días' },
  ...
  const duracionRango = document.querySelector('#duracionRango');
  const duracionTexto = document.querySelector('#duracionTexto');
  const duracionDiasReal = document.querySelector('#duracionDiasReal');

  if (duracionRango && duracionTexto && duracionDiasReal) {
    function actualizarDuracion() {
      const paso = pasosDuracion[Number(duracionRango.value)];
      duracionTexto.textContent = paso.texto;
      duracionDiasReal.value = paso.dias;
    }
    duracionRango.addEventListener('input', actualizarDuracion);
  ...
  function aplicarTemaDeporte() {
    document.querySelectorAll('[data-deporte]').forEach((elemento) => {
      ...
      // El ícono puede estar en el propio elemento (detalle.html) o en un
      // hijo .resultado-img (tarjetas de busqueda.html)
      const caja = elemento.matches('.resultado-img, .detalle-hero-img')
        ? elemento
        : elemento.querySelector('.resultado-img, .detalle-hero-img');
  ...
  if (fechasContenedor) {
    ...
    if (linkVolver) linkVolver.href = 'detalle.html?id=' + idTorneo;
